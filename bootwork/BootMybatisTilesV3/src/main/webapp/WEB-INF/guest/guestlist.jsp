...
function guest_list()
{
	   $.ajax({
		   ...
		   success:function(data){
			   let s=`<b> \${data.length}개의 글이 있습니다<b> <br>`;
			   $.each(data,function(idx,ele){
				  ...
				  if(ele.photos.length>0){
					  $.each(ele.photos,function(i,sphoto){
						  s+=`<img class="small" src="${stpath}/\${sphoto}" data-bs-toggle="modal" data-bs-target="#myPhotoLargeModal"
...
<body>
	...
	<c:if test="${sessionScope.loginok!=null }">
		<div class="gusetformarea" style="width: 400ox;">
			<textarea style="width: 100%; height: 120px;" id="gcontent"
				class="form-control"></textarea>
			<br> <input type="file" id="photoupload" multiple="multiple">
			<button type="button" id="btnaddguest" class="btn btn-sm btn-info">등록</button>
		</div>
		<hr>
	</c:if>
	<div class="gusetlistarea">
