...
<link href="https://fonts.googleapis.com/css2?family=Caveat:wght@400..700&family=Dancing+Script:wght@400..700&family=East+Sea+Dokdo&family=Jua&family=Gaegu&family=Gamja+Flower&family=Pacifico&family=Single+Day&display=swap" rel="stylesheet">
   ...
   $(function(){
	  ...
	  $("#btnansweradd").click(function(){
	   ...
	   $(".adel").click(function(){
		   alert("삭제");
		   location.href='board/deleteanswer?aidx='+this.attr("aidx");
		   answer_list();
		   
	   })
	    $(document).on("click",".adel",function(){
		   let a=confirm("해당 댓글을 삭제할까요?")
		   let aidx=$(".adel").attr("aidx");
		   if(a){
			   $.ajax({
				 	 type:"get",
				 	 dataType:"text",
		   ...
		   success:function(data){
			   ...
			   $.each(data,function(idx,ele){
				   s+=
					   `
					   \${ele.writer}(\${ele.myid})
					   <span class="aday">\${ele.writeday}`;
					   //로그인 중이면서 현재 로그인된 아이디가 댓글의 아이디와 같을경우 휴지통 나옴
					   if(loginok=='yes'&&loginid==ele.myid){
						   s+=`<i class="bi bi-trash adel" aidx="\${ele.aidx}" style="float:right; color:black; cursor:pointer"></i>`
