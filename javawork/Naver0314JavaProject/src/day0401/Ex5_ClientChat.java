...
public class Ex5_ClientChat extends JFrame implements ActionListener,Runnable{
	...
	//서버가 보내오는 메시지를 받아서 출력하기 위한 쓰레드 메서드
	@Override
	public void run() {
		// TODO Auto-generated method stub
		System.out.println("클라이언트 run");
		//서버가 보내는 메세지를 항상 대기하고 있다가 바로바로 받아서 채팅창에 출력해야한다.
		while (true)
		{
			//서버가 보내는 메세지 읽기
			try {
				...
				//채팅창에 출력
				area.append(msg+"\n");
				this.autoScroll();//항상 마지막 문자열이 보이게하기
			} catch (IOException e) {
	...
	//메시지 전송 텍스트필드와 전송버튼을 처리하기위한 메서드
	@Override
	public void actionPerformed(ActionEvent e) {
		// TODO Auto-generated method stub
		Object ob=e.getSource();// 이벤트가 발생한 소스
		//메세지 입력창이거나 전송버튼일경우 서버로 메세지 보내기
	...
	btnConnect.addActionListener(new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			// TODO Auto-generated method stub
			String serverIp=JOptionPane.showInputDialog("접속한 서버 아이피를 입력하세요.", "192.168.0.95");//초기값은 로컬 IP
			nickName=JOptionPane.showInputDialog("젒속할 닉네임을 입력하세요.", "손가원");//초기값은 각자 자기 이름으로
			...
			try {
				...
				//서버로 "1|닉네임" 이 형태로 보내기
				pw.write("1|"+nickName+"\n");
				pw.flush();//flush 를 해야 비로소 서버로 전송이된다.
